# 任务 #25b：客户档案抽取（Phase 2+ 推迟）

> **⚠️ 状态：推迟到 Phase 2+ 启用**
>
> lean MVP 阶段不实施。当前 W2 走 [#25 v2 产品 KB 抽取](../W2-D2-25-product-kb-metadata-extract.md)（产品资料库版本，不抽客户名）。
>
> 本 spec 保留作为 Phase 2 客户档案数据真正进来时的实施起点（销售/售前/实施/售后历史文档绑客户的场景）。
>
> 启用前置条件：
> - CRM 厂家选定 + 客户主数据导入 customer 表（#41 完成）
> - 销售/售前/实施/售后真实历史文档就位（合同/方案/工单/实施记录）
> - lean MVP W4 全员开放测试反馈验证「客户档案抽取确实是 P0 需求」
>
> 原 commit 来源：`4c3c6ad` (2026-06-05)

---

# 任务 #25b（原 #25 v1）：客户档案元数据抽取脚本

> **版本**：v1.0（recovered from `4c3c6ad`）
> **创建日期**：2026-06-05
> **预估工时**：1 工作日（启用时）
> **前置任务**：#24 客户主数据（customer_alias 模糊匹配）+ #19 LLM 抽象层 + #41 客户档案数据就位
> **后续任务**：客户档案文档批量抽取（启用时编号）
> **优先级**：🟢 Phase 2+ 启用时高（lean MVP 阶段不实施）
